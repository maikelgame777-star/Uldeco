...
export function ValueProps() {
  return (
    <section id="nosotros" className="py-24 relative bg-transparent">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9IiNkMGU5ZWUiLz48L3N2Zz4=')] opacity-30" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-5 sticky top-24">
            ...
            >
              <h2 className="font-display text-4xl md:text-5xl font-bold tracking-tight text-teal-900 mb-6 leading-tight">
                La perfección <br />
                <span className="text-olive-500">está en los detalles.</span>
              </h2>
              <p className="text-lg text-teal-700/70 leading-relaxed mb-8 font-light">
              ...
              <div className="flex items-center gap-4 text-sm font-medium text-teal-900">
...
function ValueCard({ title, description, icon: Icon, delay = 0 }: { title: string, description: string, icon: any, delay?: number }) {
  ...
  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    ...
    glowRef.current.style.background = `radial-gradient(400px circle at ${x}px ${y}px, rgba(11,79,92,0.07), transparent 40%)`;
  };
  ...
  return (
    <motion.div
      ...
      className="group relative p-8 bg-white border border-teal-100 rounded-3xl hover:-translate-y-2 hover:shadow-xl hover:shadow-teal-100/50 transition-all duration-500 cursor-pointer overflow-hidden shadow-md"
    >
      ...
      <div className="relative z-10">
        <div className="w-14 h-14 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-olive-50 group-hover:border-olive-200 transition-all duration-500">
          <Icon className="w-6 h-6 text-teal-700 group-hover:text-olive-600 transition-colors duration-500" />
        </div>
        <h3 className="text-xl font-semibold text-teal-900 mb-3 tracking-tight">{title}</h3>
        <p className="text-teal-700/70 leading-relaxed font-light">{description}</p>
      </div>
    </motion.div>
  );
}
