    ...
    <section id="galeria" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <span className="text-xs font-semibold uppercase tracking-widest text-olive-600 mb-2 block">Proyecto real</span>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-teal-900 leading-tight">
              {project.title}
            </h2>
            <p className="text-teal-600/70 mt-2">{project.location}</p>
          </div>
          <p className="text-teal-700/70 font-light max-w-md leading-relaxed">{project.description}</p>
        </div>
        ...
        >
          {images.map((img, i) => (
            <motion.div
              ...
              className={`group relative overflow-hidden rounded-2xl cursor-pointer bg-teal-50 shadow-md hover:shadow-xl transition-shadow ${
                tab === 'despues' ? 'sm:col-span-2 lg:col-span-4' : ''
              }`}
            >
              <img
                src={img.src}
              ...
              {tab === 'antes' && (
                <span className="absolute top-3 left-3 bg-white/90 text-teal-800 text-xs font-semibold px-2.5 py-1 rounded-full">Antes</span>
              )}
              {tab === 'despues' && (
                <span className="absolute top-3 left-3 bg-olive-500 text-white text-xs font-semibold px-2.5 py-1 rounded-full">Resultado final</span>
              )}
            </motion.div>
          ))}
        </motion.div>
        ...
        {lightbox && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-teal-950/90 z-50"
            />
            <motion.div
